...
void obj_detector(
		 ...
		 float res[NUM_CLASSES], float exp[NUM_CLASSES]
		 );
...
//  }
  ...
  for(int i = 0; i < 20; i++) {
	  ...
	  for(int j = 0; j < N; j++) {
		  for(int k = 0; k < N; k++) {
			  in_mat_a[j][k] = (float) buff[j * N + k];
			  //printf("%f ",in_mat_a[j][k]);
		  }
		  //printf("\n");
	  }
	  ...
	  // The CDMA IPs are connected through incoherent ports to main memory
  	  ...
  	  obj_detector(in_mat_a, in_mat_w0, in_mat_w1, hw_result, ex);
  }
  XTime_GetTime(xtime);
  float elapsed_time = ((double)(*xtime)/CLOCKS_PER_SEC);
	...
	for(int i = 0; i< NUM_CLASSES; i++) {
		hw_result[i] = hw_result[i] / temp;
	}
	for(int i = 0; i< NUM_CLASSES; i++) {
		ex[i] = exp(hw_result[i]);
	}
	for(int i = 0; i< NUM_CLASSES; i++) {
		temp += ex[i];
	}
	...
	printf("%f %f\n", ex[0], ex[1]);
	  printf("execution time for 20 images = %f\n", elapsed_time / 20);
}
...
void obj_detector(
		 ...
		 float w0[NUM_NEURONS][M][M],
		 float w1[NUM_CLASSES][NUM_NEURONS*((N-M+1)/2)*((N-M+1)/2)],
		 float res[NUM_CLASSES], float ex[NUM_CLASSES]
		)
{
...
int initialize_sdcard() {
	...
	result = f_open(&images, "images1.bin", FA_READ);
	if (result != 0) {
